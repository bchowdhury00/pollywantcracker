...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
    int n;
    for (n = 0; n < (polygons->lastcol - 2); n += 3){
        double surfnorm[3];
        double * surf = &surfnorm;
        double view[3];
        normalize(&view);
        surf = calculate_normal(polygons,n,surf);
        if (surfnorm[2] > 0){
            draw_line(
                polygons->m[0][n],polygons->m[1][n],
                polygons->m[0][n + 1],polygons->m[1][n + 1],
                s,c);
            draw_line(
                polygons->m[0][n + 1],polygons->m[1][n + 1],
                polygons->m[0][n + 2],polygons->m[1][n + 2],
                s,c);
            draw_line(
                polygons->m[0][n + 2],polygons->m[1][n + 2],
                polygons->m[0][n],polygons->m[1][n],
...
void add_box( struct matrix * polygons,
  ...
  add_polygon(polygons, x0, y1, z1, x1, y1, z1, x1, y0, z1);
